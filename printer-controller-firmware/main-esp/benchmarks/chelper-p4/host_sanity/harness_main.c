// Kinematics benchmark harness: exercises Klipper's chelper trapq +
// itersolve + stepcompress path (the code that would need to run on
// main-esp's P4) against a synthetic-but-representative FDM move
// sequence, and times how long the CPU-heavy step generation takes.
//
...
// High-resolution wall clock, in seconds. clock() on Windows only has
// ~15ms resolution, too coarse for the trapq_append phase below.
static double
now_seconds(void)
{
#ifdef _WIN32
...
// Compute a trapezoidal (or triangular, if too short to reach cruise_v)
// accel/cruise/decel split for a 0-to-0 velocity move of given distance.
static void
...
main(void)
{
    ...
    for (int i = 0; i < NUM_SEGMENTS; i++) {
        // Cheap deterministic pseudo-random generator (portable, no rand()
        // seeding surprises across platforms).
        ...
        if (i % 250 == 249) {
            // Occasional layer lift.
            axr_z = 1.0;
            distance = 0.2;
            cruise_v = 10.0;
        } else if (i % 2 == 0) {
            // Infill-style X line, length 0.4-20mm, speed 20-120mm/s.
            axr_x = (i % 4 == 0) ? 1.0 : -1.0;
            distance = 0.4 + r1 * 19.6;
            cruise_v = 20.0 + r2 * 100.0;
        } else {
            // Short Y step between lines.
            axr_y = 1.0;
            distance = 0.4;
            cruise_v = 20.0;
        }
    ...
    // Exercise the actual CPU-heavy path: generate step times for the
    // whole move sequence in one flush, same computation Klipper would
    // do across many small periodic flush calls.
    ...
    if (rx || ry || rz) {
        fprintf(stderr, "itersolve_generate_steps error: x=%d y=%d z=%d\n"
               , rx, ry, rz);
        return 1;
    }
    ...
    printf("=== chelper kinematics benchmark (host sanity build) ===\n");
    printf("segments (G-code-equivalent moves): %d\n", NUM_SEGMENTS);
    printf("trapq sub-moves appended:           %d (accel/cruise/decel legs)\n"
          , NUM_SEGMENTS); // trapq_append emits up to 3 list nodes per call
    ...
    printf("\n");
    printf("NOTE: this is a HOST-NATIVE sanity build (x86_64), not the P4.\n"
          "Use these numbers only to confirm the harness logic and move\n"
          "generation produce sane output. The number that matters for the\n"
          "open question is the equivalent run cross-compiled for esp32p4.\n");
