...
//   - Only G0/G1 are acted on. Everything else (G28 homing, G92 offset,
//     M-codes, etc.) is silently ignored -- this proves the gcode ->
//     kinematics -> steps chain works end to end, it is not a gcode
//     interpreter.
...
// a G92 if a caller ever needs to set a different origin (not
// implemented yet, see this header's top comment).
...
// malformed G1 line (gcode-parser would normally have already rejected
// that before this ever saw it) -- an ignored (non-G0/G1) command
// returns true, since ignoring it is expected behavior, not an error.
