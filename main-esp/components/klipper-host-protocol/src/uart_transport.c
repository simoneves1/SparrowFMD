// See khp_uart_transport.h -- unverified against real hardware.
#include "khp_uart_transport.h"
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"

#define KHP_UART_RX_BUF_SIZE 512
#define KHP_UART_TX_BUF_SIZE 0 // 0: transmit is blocking, no separate TX ring
...
bool
khp_uart_transport_init(struct khp_transport *out
                        , const struct khp_uart_transport_config *cfg)
{
    ...
    if (uart_set_pin(cfg->port, cfg->tx_pin, cfg->rx_pin
        ...
        return false;
    if (uart_driver_install(cfg->port, KHP_UART_RX_BUF_SIZE
                            , KHP_UART_TX_BUF_SIZE, 0, NULL, 0) != ESP_OK)
        return false;
